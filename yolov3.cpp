...
YOLO::YOLO(const std::string bmodel) {
  ...
  // valid object id
  int valid_kind[2] = {0, 2};
  for (size_t i = 0; i < sizeof(valid_kind) / sizeof(int); i++) {
    const int kind = valid_kind[i];
    valid_obj_id_.insert(std::make_pair(kind, kind));
  }
}
...
void YOLO::segment(cv::Mat cvimg, bm_image& in, std::vector<bm_image>& out,
                                        int x_start, int y_start) {
  bmcv_rect_t crop_rect[4];
  x_start_ = x_start;
  y_start_ = y_start;
  img_cols_ = cvimg.cols;
  img_rows_ = cvimg.rows;
  int crop_num = 0;
  for (int i = 0; i < 4; i++) {
    crop_rect[i].start_x = (x_start + i) * x_stride_;
    crop_rect[i].start_y = y_start * y_stride_;
    crop_rect[i].crop_w = net_w_;
    crop_rect[i].crop_h = net_h_;
    bm_image_create(bm_handle_, net_h_, net_w_,
             FORMAT_RGB_PLANAR, DATA_TYPE_EXT_1N_BYTE, &out[i], NULL);
    if ((crop_rect[i].start_x + crop_rect[i].crop_w) >= cvimg.cols) {
      crop_rect[i].start_x = cvimg.cols - crop_rect[i].crop_w;
    }
    if ((crop_rect[i].start_y + crop_rect[i].crop_h) >= cvimg.rows) {
      crop_rect[i].start_y = cvimg.rows - crop_rect[i].crop_h;
    }
    if (crop_rect[i].start_x >= cvimg.cols ||
             crop_rect[i].start_y >= cvimg.rows ) {
      break;
    }
    crop_num += 1;
#if 1
    cv::Rect rc;
    rc.x =  crop_rect[i].start_x;
    rc.y =  crop_rect[i].start_y;
    rc.width = crop_rect[i].crop_w;
    rc.height = crop_rect[i].crop_h;
    cv::rectangle(cvimg, rc, cv::Scalar(255, 0, 0), 2, 1, 0);
#endif
  }
  bmcv_image_vpp_convert(bm_handle_, crop_num,
                in, reinterpret_cast<bm_image*>(&out[0]),
                reinterpret_cast<bmcv_rect_t*>(crop_rect));
}

void YOLO::preForward(std::vector<cv::Mat>& images, int start_x, int start_y) {
  vector<bm_image> segment_imgs(4);
  images_.clear();
  for (size_t i = 0; i < images.size(); i++) {
    bm_image bmimg;
    bm_image_from_mat(bm_handle_, images[i], bmimg);
    segment(images[i], bmimg, segment_imgs, start_x, start_y);
    bm_image_destroy(bmimg);
    images_.push_back(images[i]);
  }
  bmcv_image_convert_to(bm_handle_, batch_size_,
             convert_attr_, &segment_imgs[0], scaled_inputs_);
  for (size_t i = 0; i < 4; i++) {
    bm_image_destroy(segment_imgs[i]);
  }
}
  ...
  for (int i = 0; i < l.w * l.h; ++i) {
    ...
    for (int n = 0; n < l.n; ++n) {
      ...
      for (int j = 0; j < l.classes; ++j) {
        ...
        dets[count].prob[j] = 0;
        if (valid_obj_id_.find(j) != valid_obj_id_.end()) {
...
std::vector<std::vector<yolov3_DetectRect> > YOLO::postForward() {
  ...
  std::vector<std::vector<yolov3_DetectRect> > det_tmp_results;
  for (int i = 0; i < batch_size_; i++) {
    ...
    detection* dets = get_detections(blobs,
                            net_w_, net_h_, &nboxes);
    std::vector<yolov3_DetectRect> det_result =
                   detection_yolov3_process(dets, nboxes,
                         net_w_, net_h_);
    free_detections(dets, nboxes);
    det_tmp_results.push_back(det_result);
  }
  int y_origin = y_start_ * y_stride_;
  if ((y_origin + net_h_) > img_rows_) {
    y_origin = img_rows_ - net_h_;
  }
  std::vector<yolov3_DetectRect> det_result_merge;
  for (int i = 0; i < batch_size_; i++) {
    int x_origin =  x_start_ * x_stride_ + i * x_stride_;
    if ((x_origin + net_w_) > img_cols_) {
      x_origin = img_cols_ - net_w_;
    }
    for (size_t j = 0; j < det_tmp_results[i].size(); j++) {
      det_tmp_results[i][j].left += x_origin;
      det_tmp_results[i][j].right += x_origin;
      det_tmp_results[i][j].top += y_origin;
      det_tmp_results[i][j].bot += y_origin;
      det_result_merge.push_back(det_tmp_results[i][j]);
    }
  }
  det_results.push_back(det_result_merge);

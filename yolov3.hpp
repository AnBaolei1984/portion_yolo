...
public:
  ...
  ~YOLO();
  void preForward(std::vector<cv::Mat>& images, int start_x, int start_y);
  void forward();
...
private:
  ...
  void segment(cv::Mat cvimg, bm_image& in, std::vector<bm_image>& out,
  ...
  /* anchor */
  float biases_[18] = {10,13,16,30,33,23,30,61,62,45,59,119,116,90,156,198,373,326};
  ...
  const int x_stride_ = 480;
  const int y_stride_ = 436;
  int x_start_;
  int y_start_;
  int img_cols_;
  int img_rows_;
  std::map<int, int> valid_obj_id_;
};
